library(readxl)
Nneka_form1 <- read_excel("Nneka_form1.xls")
View(Nneka_form1)



Perception_propt<-
  read_excel("Nneka_form1.xls")
library(tidyverse)
library(FactoMineR)
library(vcd)
library(factoextra)
...
trt1<-Common_MisConcept_About_Infertility%>%group_by(yes_no)%>%
  ...
  mutate(Fertile=str_replace_na(Fertile,"0"),
         Infertile=str_replace_na(Infertile,"0"),p_value=c("0.10","","","","",""))
...
trt2<-Common_MisConcept_About_Infertility%>%
  ...
  mutate(an2=str_replace_na(an2,"Not Selected"),Fertile=str_replace_na(Fertile,"0"),
         Infertile=str_replace_na(Infertile,"0"),p_value=c("0.06","","","","",""))%>%column_to_rownames(var="an2")
...
trt4<-Common_MisConcept_About_Infertility%>%
  ...
  mutate(an4=str_replace_na(an4,"Not Selected"),Fertile=str_replace_na(Fertile,"0"),
         Infertile=str_replace_na(Infertile,"0"),p_value=c("0.31","","",""))%>%
  column_to_rownames(var="an4")
trt4$Fertile<-as.integer(trat4$Fertile)
trt4$Infertile<-as.integer(trat4$Infertile)

Misconception_Infertility<-bind_rows(ans_1=trt1,ans_2=trt2,ans_3=trt3,
                       ans_4=trt4,.id = "Variables") 

circles1<-Misconception_Infertility%>%rownames_to_column("Treatment_Option")%>%
...
fig1 <- fig %>% layout(title = 'Treatment Options Known to Infertile Respondent',
...
fig <- figa %>% layout(title = 'Treatment Options Known to Fertile Respondent',
                       xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
                       yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))
fig
...
Known_cause<-bind_rows(ans_1=tret1,ans_2=tret2,ans_3=tret3,
                            ans_4=tret4,ans_5=tret5,ans_6=tret6,ans_7=tret7,
...
fig4 <- fig4a %>% layout(title = 'Treatment Options Known to Infertile Respondent',
                         xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
                         yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))
...
fig5 <- fig5a %>% layout(title = 'Treatment Options Known to Fertile Respondent',
                         xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
                         yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))
fig5
...
treat1<-Treatment_Options_Know%>%group_by(yes_no)%>%
 ...
 Treatment_Option<-bind_rows(ans_1=treat1,ans_2=treat2,ans_3=treat3,
...
Table1_on_trtmt_option_Knowledge<-bind_rows(Do_Know_Infertility_Start_1Year=Do_Know_Infertility_Start_1Year,
